...
Endpoints para progresso em tempo real da análise - ERRO 404 CORRIGIDO
"""
...
class ProgressTracker:
    """Rastreador de progresso em tempo real CORRIGIDO"""
...
@progress_bp.route('/start_tracking', methods=['POST'])
def start_tracking():
    """Inicia rastreamento de progresso CORRIGIDO"""
    try:
        data = request.get_json()
        session_id = data.get('session_id')
...
@progress_bp.route('/get_progress/<session_id>', methods=['GET'])
def get_progress(session_id):
    """Obtém progresso atual da análise CORRIGIDO"""
    try:
        if session_id not in progress_sessions:
            return jsonify({
                'error': 'Sessão não encontrada',
...
@progress_bp.route('/poll_updates/<session_id>', methods=['GET'])
def poll_updates(session_id):
    """Polling para atualizações de progresso CORRIGIDO"""
    try:
        if session_id not in progress_queues:
            return jsonify({
                'error': 'Sessão não encontrada'
...
@progress_bp.route('/update_progress', methods=['POST'])
def update_progress():
    """Atualiza progresso (usado internamente) CORRIGIDO"""
    try:
        data = request.get_json()
        session_id = data.get('session_id')
        step = data.get('step')
...
@progress_bp.route('/complete_analysis', methods=['POST'])
def complete_analysis():
    """Marca análise como completa CORRIGIDO"""
    try:
        data = request.get_json()
        session_id = data.get('session_id')
...
@progress_bp.route('/get_detailed_logs/<session_id>', methods=['GET'])
def get_detailed_logs(session_id):
    """Obtém logs detalhados da análise CORRIGIDO"""
    try:
        if session_id not in progress_sessions:
            return jsonify({
                'error': 'Sessão não encontrada'
...
@progress_bp.route('/active_sessions', methods=['GET'])
def get_active_sessions():
    """Lista sessões ativas de progresso CORRIGIDO"""
    try:
        active = []
        current_time = time.time()
...
# Rotas de compatibilidade para evitar 404
@progress_bp.route('/<session_id>', methods=['GET'])
def get_progress_compatibility(session_id):
    """Rota de compatibilidade para evitar 404"""
    return get_progress(session_id)

@progress_bp.route('/session/<session_id>', methods=['GET'])
def get_session_progress(session_id):
    """Rota alternativa para progresso da sessão"""
    return get_progress(session_id)